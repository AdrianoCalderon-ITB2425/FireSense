...
envsubst '${INFLUX_TOKEN} ${CHIRPSTACK_KEY} ${MAPTILER_KEY} ${CESIUM_TOKEN}' \
  ...
  > /usr/share/nginx/html-espurna/js/config.js 2>/dev/null || echo "skip espurna/config.js"
envsubst '${INFLUX_TOKEN} ${CHIRPSTACK_KEY} ${MAPTILER_KEY} ${CESIUM_TOKEN}' \
  < /usr/share/nginx/html-espurna/js/map.js.template \
  > /usr/share/nginx/html-espurna/js/map.js 2>/dev/null || echo "skip espurna/map.js"
...
envsubst '${INFLUX_TOKEN} ${CHIRPSTACK_KEY} ${MAPTILER_KEY} ${CESIUM_TOKEN} ${MAPBOX_TOKEN}' \
  ...
  > /usr/share/nginx/html-espurna/js/rf_coverage.js 2>/dev/null || echo "skip espurna/rf_coverage.js"
