# Architecture Index - 01-architecture/

## Quick Navigation

| **Document** | **Description** | **Quick Access** |
|---|---|---|
| **01-k8s** | Complete K8s architecture | [Read →](./01-k8s.md) |
| **02-lorawan** | LoRaWAN architecture | [Read →](./02-lorawan.md) |
| **03-playtest-k8s** | K8s playtest & validation | [Read →](./03-playtest-k8s.md) |
| **04-sprints-technical-doc** | Sprints technical documentation | [Read →](./04-sprints-technical-doc.md) |
| **05-diagrams** | Architecture diagrams | [Read →](./05-diagrams.md) |
| **06-runbooks** | Operational runbooks | [Read →](./06-runbooks.md) |
| **07-sustainability-plan** | Sustainability & digital transformation | [Read →](./07-sustainability-plan.md) |
| **08-project-memory** | Complete project memory | [Read →](./08-project-memory.md) |
| **09-nodered-apis** | Node-RED flows & external APIs | [Read →](./09-nodered-apis.md) |

---

## Main Index

[Home](../../README.md)
